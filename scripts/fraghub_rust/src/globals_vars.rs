...
pub static IN_SILICO_PATTERN: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)in[-_ ]?silico|predict").unwrap());
pub static MS_LEVEL_PATTERN: Lazy<Regex> = Lazy::new(|| Regex::new(r"(\d+)").unwrap());
pub static RETENTION_TIME_PATTERN: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)^([\d\.]+)\s*(m|min|minute|minutes|s|sec|second|seconds|ms|millisecond|milliseconds)?$").unwrap());
